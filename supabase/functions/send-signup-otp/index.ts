...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: recent } = await supabase.from("signup_otp_codes").select("created_at").eq("email", email).order("created_at", { ascending: false }).limit(1);
    ...
    const { error: dbError } = await supabase.from("signup_otp_codes").insert({ email, code_hash: codeHash, expires_at: expiresAt });
    ...
    if (!termiiRes.ok) throw new Error("Termii error: " + JSON.stringify(termiiData));
